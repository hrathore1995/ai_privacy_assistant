...
* **Hybrid detection** combining regex, spaCy NER, and an LLM for context‑dependent PII.
...
* **Detectors**
  ...
  * **LLM** to catch context‑dependent PII and add explanations.
...
* **Layout‑Preserving Redaction**
  ...
  * Draw **black boxes** on the **original PDF** (visual overlay).
...
* **UI/UX**
  ...
  * Clean theme, drag‑and‑drop upload, activity log, toasts.
  * Overlay toggle auto‑locks Mode to `redact` and disables “Include report” (since overlay preserves the original).
* **Reports**
...
* **Dockerized**
  ...
  * `docker compose up --build` — reproducible without local setup.
...
│   ├── pii_detector.py        # regex + spaCy + LLM
...
* The Dockerfile installs **`en_core_web_md`** for faster builds. You can switch both the Dockerfile and env to `en_core_web_lg` if you need extra accuracy.
...
2. Click **Analyze PII** to see findings:
   ...
   * **LLM note**: contextual insights on detected PII
...
* **Local‑first core**: Regex and spaCy run locally; OCR runs locally.
* **LLM**: The system uses an LLM; text chunks may be sent to the provider. Ensure this aligns with your data policy.
...
```bash
# Local
aaa python3 -m venv venv && source venv/bin/activate
pip install -r requirements.txt
